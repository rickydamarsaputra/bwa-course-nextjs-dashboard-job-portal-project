...
export const supabaseGetPublicUrl = (filename: File | string, bucket: 'company' | 'applicant') => {
...
export const supabaseDeleteFile = async (filename: File | string, bucket: 'company' | 'applicant') => {
  const { data, error } = await supabaseClient
  .storage
  .from(bucket)
