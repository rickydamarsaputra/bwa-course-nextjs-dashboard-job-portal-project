...
const JobDetail: FC<JobDetailProps> = ({ detail }) => {
  ...
  const requiredSkills: any = detail?.requiredSkills;

  return(
    <div>
      <div className="grid grid-cols-3 w-full gap-5">
        <div className="col-span-2 space-y-10 my-6">
          <div>
            <div className="text-3xl font-semibold">
              Description
            </div>
            <div className="text-gray-500 mt-4" dangerouslySetInnerHTML={{ __html: detail?.description!! }}></div>
          </div>
          <div>
            <div className="text-3xl font-semibold">
              Responsibilities
            </div>
            <div className="text-gray-500 mt-4" dangerouslySetInnerHTML={{ __html: detail?.responsibility!! }}></div>
          </div>
          <div>
            <div className="text-3xl font-semibold">
              Who You Are
            </div>
            <div className="text-gray-500 mt-4" dangerouslySetInnerHTML={{ __html: detail?.whoYouAre!! }}></div>
          </div>
          <div>
            <div className="text-3xl font-semibold">
              Nice-To-Haves
            </div>
            <div className="text-gray-500 mt-4" dangerouslySetInnerHTML={{ __html: detail?.niceToHaves!! }}></div>
          </div>
        </div>
        <div>
          <div className="text-3xl font-semibold">
